  ...
  async getPartners(user: UserDocument, limit: number, offset: number) {
    ...
    for (const s of sessions) {
      ...
      if (!partnerMap[partnerId]) {
        partnerMap[partnerId] = { sessions: 0, lastSessionAt: null, ratings: [] };
      }
      partnerMap[partnerId].sessions++;
      if (!partnerMap[partnerId].lastSessionAt || s.completedAt > partnerMap[partnerId].lastSessionAt) {
    ...
    const partnerIds = Object.keys(partnerMap)
      .sort((a, b) => (partnerMap[b].lastSessionAt > partnerMap[a].lastSessionAt ? 1 : -1))
    ...
    const partners = partnerIds.map((id) => {
      ...
      return {
        user: u
          ? { id: u._id, name: u.name, avatar: u.avatar || null, trustScore: u.trustScore }
